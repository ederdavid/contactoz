class Notifier < ActionMailer::Base
  def password_reset_instructions(user)
    ...
    sent_on      Time.now
    body         :edit_password_reset_url => edit_password_reset_url(:id => user.perishable_token, :host => "kontactoz.com")
  end
  def activation_instructions(user)
    ...
    recipients    user.email
    sent_on       Time.now
    body          :account_activation_url => activate_url(:activation_code => user.perishable_token, :host => "kontactoz.com")
  end
  def welcome(user)
